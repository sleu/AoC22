...
def draw_map():
    width = map_width+1
    bmap_length = map_length-1
    for y in range(bmap_length):
...
def sand():
    ...
    for y, _ in enumerate(map):
        if y == len(map)-1: 
            print("Infinite reached")
            return 1 #reached bottom, infinite
...
for l in input_list:
        for c in l:
            if c[1] > map_length: map_length = c[1] #bottom
map_length +=3
